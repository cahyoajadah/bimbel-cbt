import { Menu as MenuIcon, Bell, LogOut, User, ChevronRight, FileWarning, Search } from 'lucide-react';
...
export const Header = () => {
  // ... (Semua logika hooks: useAuthStore, useQuery, mutations TETAP SAMA)
  const { user, logout } = useAuthStore();
  const { toggleSidebar } = useUIStore();
  const navigate = useNavigate();
  const queryClient = useQueryClient();

  // ... (Query fetching data notifikasi TETAP SAMA)
  ...
  // ... (Logika logout dan fungsi bantuan TETAP SAMA)
  ...
  return (
    /* UBAH HEADER STYLE:
       1. `sticky top-0` diubah menjadi `sticky` dengan `top-0` untuk mobile dan `lg:top-4` untuk desktop.
       2. Ditambahkan `lg:mx-8 lg:rounded-2xl` agar mengapung di tengah pada layar besar.
       3. Background diubah jadi `bg-white/80 backdrop-blur-xl` (efek kaca/glassmorphism).
       4. Border bawah dihapus pada mode desktop.
       5. Shadow diperhalus `shadow-sm lg:shadow-lg lg:shadow-blue-900/5`.
    */
    <header className={clsx(
      "sticky z-30 transition-all duration-300",
      // Mobile view: full width, top 0
      "top-0 border-b border-gray-100 bg-white/95 backdrop-blur-sm",
      // Desktop view: floating, rounded, glass effect
      ...
      <div className="flex items-center justify-between h-20 px-4 sm:px-6 lg:px-8">
        ...
        {/* Left Section: Mobile toggle & Search bar placeholder */}
        ...
        <div className="flex items-center gap-3 sm:gap-6">
          ...
          {/* Notification Bell - Style diperbarui */}
          {showBell && (
            <Popover className="relative">
              {({ open, close }) => (
                <>
                  <Popover.Button 
                    className={clsx(
                      "relative p-3 rounded-xl transition-all duration-200 focus:outline-none group overflow-hidden", 
                      open ? "bg-blue-100 text-blue-700" : "text-gray-500 hover:bg-blue-50 hover:text-blue-600"
                    )}
                  >
                    ...
                    {notifications?.length > 0 && (
                      <span className="absolute top-2 right-2 block h-2.5 w-2.5 rounded-full bg-red-500 ring-2 ring-white animate-bounce" />
                    )}
                  </Popover.Button>

                  {/* Popover Panel Style (Diperhalus, sudut lebih bulat) */}
                  <Transition
                    as={Fragment}
                  ...
                  >
                    <Popover.Panel className="absolute right-0 z-50 mt-4 w-80 sm:w-96 transform px-4 sm:px-0 origin-top-right">
                      <div className="overflow-hidden rounded-3xl shadow-2xl shadow-blue-900/20 ring-1 ring-black ring-opacity-5 bg-white border border-gray-50">
                         {/* ... (Isi Popover TETAP SAMA, hanya style container luarnya yang diubah) ... */}
                        <div className="p-0">
                            ...
                            <div className="max-h-[350px] overflow-y-auto custom-scrollbar bg-gray-50/30">
                                {notifications && notifications.length > 0 ? (
                                    notifications.map((item) => (
                                        // Style item notifikasi diperhalus
                                        <div 
                                            key={item.id} 
                                            onClick={() => handleViewAll(close)}
                                            className="group relative flex flex-col gap-1 px-6 py-4 hover:bg-white transition-all cursor-pointer border-b border-gray-100 last:border-0 hover:shadow-sm"
                                        >
                                             {/* Indikator biru di kiri saat hover */}
                                            ...
                                            {user.role === 'siswa' ? (
                                               ...
                                               // (Bagian pembuat soal tetap sama)
                                                <>
                                      ...
                                      <div className="p-4 bg-gray-100 rounded-full">
                                        <Bell size={32} className="opacity-40 text-gray-500"/>
                                      </div>
                            ...
                            <div className="p-4 bg-white border-t border-gray-50/50">
                                <button 
                                  className="flex w-full items-center justify-center gap-2 px-4 py-3 text-sm font-bold text-white bg-gradient-to-r from-blue-600 to-blue-500 rounded-xl shadow-blue-200 hover:shadow-blue-300 hover:scale-[1.02] transition-all active:scale-[0.98]"
                                  onClick={() => handleViewAll(close)}
                                >
          ...
          <Menu as="div" className="relative">
            <Menu.Button className="group flex items-center gap-3 focus:outline-none p-1.5 pr-4 rounded-full transition-all border border-transparent hover:border-gray-200 hover:bg-white hover:shadow-sm">
              <div className="relative">
                {/* Avatar dengan gradient dan ring */}
              ...
              <ChevronRight size={16} className="text-gray-300 group-hover:text-blue-500 transition-colors hidden md:block rotate-90 group-hover:rotate-[270deg]" />
            </Menu.Button>

            {/* Dropdown Menu Style (Sudut lebih bulat, bayangan lebih dalam) */}
            <Transition
              as={Fragment}
                      ...
                      <Menu.Item>
                        {({ active }) => (
                          <button
                            ...
                            className={clsx('flex w-full items-center px-4 py-3 text-sm font-medium rounded-xl transition-colors group', active ? 'bg-blue-50 text-blue-700' : 'text-gray-700 hover:bg-gray-50')}
                          >
                  ...
                  <Menu.Item>
                    {({ active }) => (
                      <button
                        ...
                        className={clsx('flex w-full items-center px-4 py-3 text-sm font-bold rounded-xl transition-colors group', active ? 'bg-red-50 text-red-600' : 'text-red-600 hover:bg-red-50/50')}
                      >
