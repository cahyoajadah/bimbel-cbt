...
export default function Materials() {
  ...
  const [search, setSearch] = useState(''); // [BARU] State Search
  ...
  const { data: materials, isLoading } = useQuery({
    queryKey: ['admin-materials', selectedSubject, search], // [BARU]
    queryFn: async () => {
      const params = { 
  ...
  // ... (Mutations create, update, delete, handlers - SAMA) ...
  const createMutation = useMutation({
    ...
    onSuccess: () => { queryClient.invalidateQueries(['admin-materials']); setIsModalOpen(false); reset(); toast.success('Materi berhasil dibuat'); },
  ...
  const updateMutation = useMutation({
    mutationFn: async ({ id, formData }) => {
      ...
      const res = await api.post(`/admin/materials/${id}`, data, { headers: { 'Content-Type': 'multipart/form-data' } });
      return res.data;
    },
    onSuccess: () => { queryClient.invalidateQueries(['admin-materials']); setIsModalOpen(false); setEditingMaterial(null); reset(); toast.success('Materi berhasil diperbarui'); },
  });
  const deleteMutation = useMutation({
    mutationFn: async (id) => { await api.delete(`/admin/materials/${id}`); },
    onSuccess: () => { queryClient.invalidateQueries(['admin-materials']); toast.success('Materi berhasil dihapus'); },
  });
  const handleOpenModal = (material = null) => {
    if (material) {
      setEditingMaterial(material);
      setValue('subject_id', material.subject_id); setValue('title', material.title); setValue('type', material.type);
      setValue('description', material.description); setValue('content_url', material.content); 
    } else {
      setEditingMaterial(null); reset({ type: 'text' });
      if (selectedSubject) setValue('subject_id', selectedSubject);
    }
    setIsModalOpen(true);
  };
  const handleDelete = (material) => {
    showConfirm({ title: 'Hapus Materi', message: `Yakin ingin menghapus materi "${material.title}"?`, confirmText: 'Hapus', type: 'danger', onConfirm: () => deleteMutation.mutate(material.id) });
  };
  const onSubmit = (data) => { if (editingMaterial) { updateMutation.mutate({ id: editingMaterial.id, formData: data }); } else { createMutation.mutate(data); } };
  ...
  return (
    <div className="space-y-6">
      ...
      {/* [PERBAIKAN] SEARCH BAR & Filter Subject */}
      <div className="flex flex-col md:flex-row gap-4 bg-white p-4 rounded-lg shadow-sm border border-gray-200">
        <div className="flex items-center space-x-2 flex-1 border rounded-md px-3 bg-gray-50">
            <Search className="w-5 h-5 text-gray-400" />
            <input 
                type="text" 
                placeholder="Cari materi..." 
                className="flex-1 border-none bg-transparent focus:ring-0 text-sm outline-none py-2"
                value={search}
      ...
      <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title={editingMaterial ? 'Edit Materi' : 'Tambah Materi Baru'}>
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          <div>
            <label className="block text-sm font-medium text-gray-700">Mata Pelajaran</label>
            <select className="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500" {...register('subject_id', { required: 'Mata pelajaran wajib dipilih' })} disabled={editingMaterial}>
          ...
          <div>
            ...
            <select className="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500" {...register('type')}>
          ...
          {watchedType === 'pdf' ? (
            <div>
              ...
              <input type="file" accept=".pdf" className="mt-1 block w-full text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-blue-50 file:text-blue-700 hover:file:bg-blue-100" {...register('content_file', { required: !editingMaterial && watchedType === 'pdf' })} />
              {editingMaterial && <p className="text-xs text-gray-500 mt-1">Biarkan kosong jika tidak ingin mengubah file.</p>}
            </div>
          ) : (
            <Input label={watchedType === 'video' ? 'Link Video (YouTube)' : 'Link Artikel / Konten'} placeholder="https://..." {...register('content_url', { required: 'Link konten wajib diisi' })} error={errors.content_url} />
          )}
          <div>
            <label className="block text-sm font-medium text-gray-700">Deskripsi (Opsional)</label>
            <textarea className="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500" rows="3" {...register('description')}></textarea>
          </div>
          <div className="flex justify-end space-x-3 pt-4 border-t mt-4">
            <Button type="button" variant="outline" onClick={() => setIsModalOpen(false)}>Batal</Button>
            <Button type="submit" loading={createMutation.isPending || updateMutation.isPending}>{editingMaterial ? 'Simpan Perubahan' : 'Buat Materi'}</Button>
          </div>
        </form>
      </Modal>
    </div>
